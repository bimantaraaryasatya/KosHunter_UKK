...
const DeleteReview = ({selectedReview, onSuccess}: {selectedReview:IReview; onSuccess: () => void}) => {
    const [isShow, setIsShow] = useState<boolean>(false)
    const [review, setReview] = useState<IReview>({...selectedReview})
    ...
    const handleSubmit = async (e: FormEvent) => {
       try {
           e.preventDefault()
           const url = `${BASE_API_URL}/review/${selectedReview.id}`
    ...
    <div>
        ...
        <Modal isShow={isShow} onClose={state => setIsShow(state)}>
            <form onSubmit={handleSubmit}>
                <div className="sticky top-0 bg-white px-5 pt-5 pb-3 shadow text-start">
